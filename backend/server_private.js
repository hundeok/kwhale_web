...
const app = express();
const PORT = Number(process.env.KWHALE_PRIVATE_API_PORT || 3340);
const privateRoot = path.resolve(process.env.KWHALE_PRIVATE_ROOT || path.join(__dirname, 'private-data'));

function resolveDatabasePath() {
  if (process.env.KWHALE_PRIVATE_DB) return path.resolve(process.env.KWHALE_PRIVATE_DB);
  const pointerPath = path.join(privateRoot, 'releases', 'latest-release.json');
  const pointer = JSON.parse(fs.readFileSync(pointerPath, 'utf8'));
  return path.join(privateRoot, pointer.database);
}
...
function parseYear(value) {
  if (String(value).toLowerCase() === 'recent') return 'recent';
...
function latestCategoryLeaders(category, year, limit = 5) {
  return statement(`
    ${snapshotCte(year)}
...
function latestAssetLeaders(year, whereSql, parameters = [], limit = 3) {
  return statement(`
    ${snapshotCte(year)}
...
function extractRegion(detail) {
  ...
  const match = normalized.match(
    /(서울특별시|부산광역시|대구광역시|인천광역시|광주광역시|대전광역시|울산광역시|세종특별자치시|경기도|강원특별자치도|강원도|충청북도|충청남도|전북특별자치도|전라북도|전라남도|경상북도|경상남도|제주특별자치도)\s+([가-힣]+(?:시|군|구))(?:\s+([가-힣0-9·.-]+(?:읍|면|동|가|리)))?/
  );
...
function parseRealEstateLocation(detail) {
  ...
  const provinceMatch = normalized.match(
    /(서울특별시|부산광역시|대구광역시|인천광역시|광주광역시|대전광역시|울산광역시|세종특별자치시|경기도|강원특별자치도|강원도|충청북도|충청남도|전북특별자치도|전라북도|전라남도|경상북도|경상남도|제주특별자치도)/
  );
  if (!provinceMatch) return { province: '기타·국외', district: '주소 미식별', locality: '주소 미식별', matched: false };
...
function realEstateSnapshot(year) {
  const key = String(parseYear(year));
  if (realEstateSnapshotCache.has(key)) return realEstateSnapshotCache.get(key);
...
function aggregateRealEstateRows(rows, keySelector) {
  ...
  for (const row of rows) {
    ...
    if (!item.topAsset || row.valuation > item.topAsset.valuation) item.topAsset = row;
    groups.set(key, item);
  }
...
function compactWon(value) {
  ...
  if (amount >= 1_000_000_000_000) {
    return `${Number((amount / 1_000_000_000_000).toFixed(2)).toLocaleString('ko-KR')}조 원`;
  }
  if (amount >= 100_000_000) {
    return `${Number((amount / 100_000_000).toFixed(1)).toLocaleString('ko-KR')}억 원`;
  }
...
function buildRealEstateAlpha(rows) {
  ...
  const districts = aggregateRealEstateRows(
    validRows.filter((row) => row.district !== '주소 미식별' && row.district !== '기타'),
    (row) => `${row.province} ${row.district}`
  );
  const localities = aggregateRealEstateRows(
    validRows.filter((row) => row.locality !== '주소 미식별' && row.locality !== '기타'),
    (row) => `${row.province} ${row.district} ${row.locality}`
  );
  const buildingRegions = aggregateRealEstateRows(
    validRows.filter((row) => row.category === '건물'), (row) => row.province
  );
  const landRegions = aggregateRealEstateRows(
    validRows.filter((row) => row.category === '토지'), (row) => row.province
  );
  const apartmentRegions = aggregateRealEstateRows(
    validRows.filter((row) => row.subcategory.includes('아파트')), (row) => row.province
  );
  const commercialRegions = aggregateRealEstateRows(
    validRows.filter((row) => /상가|근린생활시설|빌딩|사무실|공장/.test(row.subcategory)),
    (row) => row.province
  );
  const landTypes = aggregateRealEstateRows(
    validRows.filter((row) => row.category === '토지'), (row) => row.subcategory
  );
  const luxuryRegions = aggregateRealEstateRows(
    validRows.filter((row) => row.valuation >= 3_000_000_000), (row) => row.province
  ).sort((a, b) => b.assetCount - a.assetCount || b.totalValuation - a.totalValuation);
  ...
  for (const row of validRows) {
    ...
    item.totalValuation += row.valuation;
    item.assetCount += 1;
    if (row.province !== '기타·국외') item.regions.add(row.province);
    officialMap.set(row.officialId, item);
  }
  const officials = [...officialMap.values()].sort((a, b) =>
    b.totalValuation - a.totalValuation || b.assetCount - a.assetCount
  );
  const diversified = [...officials].sort((a, b) =>
    b.regions.size - a.regions.size || b.totalValuation - a.totalValuation
  )[0];
  ...
  const rowsOut = [
    ...
    insight('luxury_cluster', '30억+ 자산 밀집지역', luxury?.name, `${luxury?.assetCount.toLocaleString()}건 · ${compactWon(luxury?.totalValuation)}`, luxury?.assetCount, { province: luxury?.name, minValue: 3_000_000_000 }, '단일 신고가액 30억원 이상 자산 수'),
...
function realEstateAlpha(year) {
  const key = String(parseYear(year));
  if (!realEstateAlphaCache.has(key)) {
    realEstateAlphaCache.set(key, buildRealEstateAlpha(realEstateSnapshot(year)));
  }
...
function buildConcentration(items, smartIds, smartPopulation, publicPopulation, minimumSmartHolders) {
  return [...items.entries()]
    ...
    .sort((a, b) =>
      Number(b.exclusiveToTopOnePercent) - Number(a.exclusiveToTopOnePercent) ||
...
function rankingInsights(rows) {
  ...
  return {
    ...
    largeIncreasePersons: profits.filter((value) => value >= 1_000_000_000).length,
    largeDecreasePersons: profits.filter((value) => value <= -1_000_000_000).length,
  };
}

function respondWithRankings(res, req, comparisonRows, mode, methodology) {
  ...
  const enriched = comparisonRows.map((row) => {
    ...
    const reliable = previousNetWorth >= 100_000_000 && intervalYears === 1;
    return {
      ...row,
      profit,
      profitRate,
      ...
      intervalYears,
      reliable,
      confidence: reliable ? 'A' : previousNetWorth >= 100_000_000 ? 'B' : 'C',
    };
  }).filter((row) => row.profitRate !== null && Number.isFinite(row.profitRate));
  const searched = searchTokens.length
    ? enriched.filter((row) => searchTokens.every((token) =>
        `${row.name} ${row.agency || ''} ${row.title || ''}`.includes(token)))
    : enriched;
  const filtered = searched.filter((row) => {
    if (lens === 'reliable') return row.reliable;
    if (lens === 'rise') return mode === 'yield' ? row.reliable && row.profit > 0 : row.profit > 0;
    if (lens === 'fall') return mode === 'yield' ? row.reliable && row.profit < 0 : row.profit < 0;
    if (lens === 'spike') return row.reliable && row.profitRate >= 100;
    if (lens === 'plunge') return row.reliable && row.profitRate <= -50;
    if (lens === 'largeup') return row.profit >= 1_000_000_000;
    if (lens === 'largedown') return row.profit <= -1_000_000_000;
    if (lens === 'smallbase') return Number(row.previousNetWorth) < 100_000_000;
    if (lens === 'multiyear') return row.intervalYears > 1;
    return true;
  });
  ...
  const driverRows = disclosureIds.length ? statement(`
  ...
  const quality = {
    reconciliationPass: enriched.every((row) =>
      Number(row.currentNetWorth) - Number(row.previousNetWorth) === Number(row.profit)),
    eligiblePersons: enriched.length,
    reliablePersons: enriched.filter((row) => row.reliable).length,
    smallBasePersons: enriched.filter((row) => Number(row.previousNetWorth) < 100_000_000).length,
    ...
    minimumReliableBase: 100_000_000,
...
app.get('/api/health', (req, res) => {
  const release = statement('SELECT * FROM dataset_release LIMIT 1').get();
  ok(res, {
    status: 'ok',
...
app.get('/api/meta/years', (req, res) => {
  ...
  ok(res, {
    ...
    allTime: {
      label: 'ALL-TIME',
      persons: statement('SELECT COUNT(*) AS count FROM person').get().count,
      definition: '인물별 가장 최근 공개 신고',
    },
  });
});

app.get('/api/meta/methodology', (req, res) => {
  const release = statement('SELECT * FROM dataset_release LIMIT 1').get();
  const metrics = Object.fromEntries(
    statement('SELECT metric_key, metric_value FROM dataset_metric').all()
      .map((row) => [row.metric_key, JSON.parse(row.metric_value)])
  );
  const artifacts = statement(`
...
app.get('/api/dashboard', (req, res) => {
  ...
  const cryptoOwnersCount = statement(`
    ${snapshotCte(year)}
    SELECT COUNT(DISTINCT d.person_id) AS count
    FROM latest_disclosures d JOIN asset a ON a.disclosure_id = d.id
    WHERE a.category = '가상자산' AND TRIM(COALESCE(a.detail, '')) <> ''
  `).get().count;
  const categoryTotal = categorySums.reduce(
    (sum, row) => sum + Number(row._sum.valuation || 0), 0
  );
  const edgyStats = {
    topDebtors: latestCategoryLeaders('채무', year),
    topCryptoWhales: latestCategoryLeaders('가상자산', year),
    topCashKings: latestCategoryLeaders('예금', year),
    topBuildingKings: latestCategoryLeaders('건물', year),
  };
  ok(res, {
    ...summary,
    topRankings,
    ...
    quality: {
      accountingPass:
        Number(summary.totalAssetsValuation) - Number(summary.totalLiabilities) ===
...
app.get('/api/dashboard/edgy-stats', (req, res) => {
  const year = parseYear(req.query.year);
  ok(res, {
    topDebtors: latestCategoryLeaders('채무', year),
    topCryptoWhales: latestCategoryLeaders('가상자산', year),
...
app.get('/api/officials', (req, res) => {
  ...
  const data = resultRows.map((row) => {
    const {
      resultPersons, resultGrossAssets, resultLiabilities, resultNetWorth,
      resultAverageNetWorth, resultHighNetCount, resultNegativeNetCount,
      resultMissingOrgCount, resultMissingTitleCount, resultHomonymCount,
      ...official
    } = row;
    return official;
  });
  ok(res, data, {
    meta: {
      ...
      quality: {
        accountingPass:
          Number(summary.grossAssets) - Number(summary.liabilities) === Number(summary.netWorth),
        snapshotPolicy: year === 'recent'
          ? `${latestCompleteYear}년 완결 모집단 + ${defaultYear}년 공개분 인물별 최신 반영`
...
app.get('/api/officials/:id', (req, res) => {
  ...
  const official = statement(`
    ...
    WHERE p.id = ?
  `).get(req.params.id);
  if (!official) return fail(res, 'Not found', 404);
  const assets = statement(`
  ...
  `).all(req.params.id);
  const previousDisclosure = disclosures.find(
    (item) => Number(item.year) < Number(official.latestYear)
  ) || null;
  ...
  const summarizeAssets = (rows) => {
    ...
    for (const row of rows) {
      ...
      if (isLiability) liabilities += valuation;
      else grossAssets += valuation;
      ...
      if ('owner' in row) {
        ...
        if (isLiability) owner.liabilities += valuation;
        else owner.grossAssets += valuation;
        owner.assetCount += 1;
  ...
  const previousCategoryMap = new Map(
    previousSummary.categories.map((item) => [item.category, item.valuation])
  );
  const currentCategoryMap = new Map(
    currentSummary.categories.map((item) => [item.category, item])
  );
  ...
  ok(res, {
    ...
    quality: {
      accountingPass:
        currentSummary.grossAssets === Number(official.totalAssets) &&
        currentSummary.liabilities === Number(official.liabilities) &&
        currentSummary.netWorth === Number(official.netWorth),
      assetCompositionPass:
        currentSummary.grossAssets === 0 ||
...
app.get('/api/rankings/:mode', (req, res) => {
  const mode = req.params.mode;
  if (!['profit', 'yield'].includes(mode)) return fail(res, 'Invalid mode', 400);
  const currentYear = parseYear(req.query.year);
  if (currentYear === 'recent') {
    const comparisonRows = statement(`
      ${snapshotCte('recent')},
      previous_ranked AS (
    ...
    `).all();
    return respondWithRankings(res, req, comparisonRows, mode, {
  ...
  if (!previousYear) return fail(res, '비교 가능한 이전 연도가 없습니다.', 409);
  const comparisonRows = statement(`
    WITH yearly AS (
      SELECT d.*, ROW_NUMBER() OVER (
...
app.get('/api/map', (req, res) => {
  ...
  for (const row of rows) {
    ...
    if (!coordinateAddressGroups.has(coordinateKey)) coordinateAddressGroups.set(coordinateKey, new Set());
    if (address) coordinateAddressGroups.get(coordinateKey).add(address);
  }
...
function instrumentStats(category, extractor, year, subcategories = [], extraCategories = []) {
  ...
  for (const row of rows) {
    ...
    for (const holding of extractedHoldings) {
      ...
      if (holding.changeType === 'ACQUIRED' || inferredAcquisition) item.acquiredPositions += 1;
      if (holding.changeType === 'INCREASED' && !inferredAcquisition) item.increasedPositions += 1;
      if (holding.changeType === 'DECREASED') item.decreasedPositions += 1;
      ...
      if (holding.quantity !== null) existingHolder.quantity += holding.quantity;
      if (valuation !== null) {
    ...
    if (rowAllocatedValuation > Number(row.valuation || 0)) overAllocatedRows += 1;
  }
...
function stockSubcategories(assetClass) {
  if (assetClass === 'unlisted') return ['비상장주식'];
  if (assetClass === 'bonds') {
    return ['기타채권', '금융채', '회사채', '국채', '채권', '공채', '지방채'];
  }
  if (assetClass === 'all') return ['상장주식', '비상장주식'];
  return ['상장주식'];
}
function stockExtraCategories(assetClass) {
  return assetClass === 'bonds' ? ['채권'] : [];
}
app.get('/api/stats/stocks', (req, res) => {
  const year = parseYear(req.query.year);
  ...
  if (!stockCache.has(cacheKey)) {
    stockCache.set(cacheKey, instrumentStats(
      '증권', extractSecurityHoldings, year,
      stockSubcategories(assetClass), stockExtraCategories(assetClass)
    ));
  }
...
app.get('/api/stats/stocks/:name', (req, res) => {
  const year = parseYear(req.query.year);
  const assetClass = ['listed', 'unlisted', 'bonds', 'all'].includes(String(req.query.class))
    ? String(req.query.class) : 'listed';
  const cacheKey = `${year}:${assetClass}`;
...
app.get('/api/stats/crypto', (req, res) => {
  const year = parseYear(req.query.year);
  if (!cryptoCache.has(year)) cryptoCache.set(year, instrumentStats('가상자산', extractCryptoHoldings, year));
  const result = cryptoCache.get(year);
  ok(res, result.items.map(({ holders, ...item }) => item), {
    year, quality: result.quality, alpha: result.alpha,
  });
});
app.get('/api/stats/crypto/people', (req, res) => {
  const year = parseYear(req.query.year);
  ...
  `).get();
  const duplicateSnapshots = statement(`
    ${snapshotCte(year)}
  ...
  `).get().count;
  const referencePerson = statement(`
    ${snapshotCte(year)}
  ...
  const peopleQuality = {
    ...
    duplicateSnapshots,
    reconciliationPass:
      Number(personAudit.groupedPersons) === Number(summary.persons) &&
...
app.get('/api/stats/crypto/:name', (req, res) => {
  const year = parseYear(req.query.year);
  if (!cryptoCache.has(year)) cryptoCache.set(year, instrumentStats('가상자산', extractCryptoHoldings, year));
  const result = cryptoCache.get(year);
  ok(res, result.items.find((item) => item.name === req.params.name)?.holders || [], {
...
app.get('/api/analysis/real-estate', (req, res) => {
  const year = parseYear(req.query.year);
...
app.get('/api/analysis/new-assets', (req, res) => {
  const year = parseYear(req.query.year);
  ...
  if (region !== 'all') {
    const regionAliases = region === '강원특별자치도'
      ...
      : region === '전북특별자치도'
        ? ['전북특별자치도', '전라북도']
        : [region];
    filterConditions.push(`a.category IN ('건물', '토지') AND (${
      regionAliases.map(() => 'a.detail LIKE ?').join(' OR ')
    })`);
  ...
  const acquisitionStats = statement(`
    ${snapshotCte(year)}
  ...
  `).get();
  const categoryBreakdown = statement(`
    ${snapshotCte(year)}
  ...
  `).all();
  const taxonomyBreakdown = statement(`
    ${snapshotCte(year)}
  ...
  const regionBreakdown = [...regionMap.values()].sort((a, b) =>
    b.eventCount - a.eventCount || a.name.localeCompare(b.name, 'ko')
  );
  const filteredCount = statement(`
    ${snapshotCte(year)}
    SELECT COUNT(*) AS total
    FROM latest_disclosures d
    JOIN person p ON p.id = d.person_id
  ...
  ok(res, {
    ...
    stats: {
      ...
      truncated: false,
      reconciliationPass:
        Number(acquisitionStats.totalDifference || 0) ===
    ...
    methodology: {
      ...
      acquiredDefinition:
        '현재 평가액=공식 증가액, 원문 신규·취득 명시, 또는 건물·토지의 신규 면적·지분 증가 신호',
      increasedDefinition:
        '공식 증가액이 현재 평가액과 다른 기존 금융·투자자산 증가 행',
      exclusions: ['채무 증가', 'difference 0 이하이면서 원문 증가 신호도 없는 행'],
      valuationPolicy: '금액 확정 이벤트와 원문 신호 전용 이벤트를 분리하며 금액을 임의 추정하지 않음',
      realEstateDifferencePolicy:
        '건물·토지는 보존 원천에 현재 평가액과 면적 증가 신호가 있으나 별도 증가액(difference)이 없어 현재 평가액을 증가액으로 대체하지 않음',
    },
  });
});

app.get('/api/analysis/hyperlocal', (req, res) => {
  const year = parseYear(req.query.year);
  const rows = statement(`
    ${snapshotCte(year)}
...
app.get('/api/analysis/real-estate-regions', (req, res) => {
  const year = parseYear(req.query.year);
  ...
  const search = String(req.query.search || '').trim().slice(0, 80);
  const allRows = realEstateSnapshot(year);
...
app.get('/api/analysis/real-estate-assets', (req, res) => {
  const year = parseYear(req.query.year);
  const category = ['건물', '토지'].includes(String(req.query.category))
    ? String(req.query.category) : 'all';
  const subcategory = String(req.query.subcategory || 'all').trim().slice(0, 80);
  ...
  const limit = parseLimit(req.query.limit, 50, 100);
  const offset = Math.max(0, Number.parseInt(req.query.offset, 10) || 0);
  const filtered = realEstateSnapshot(year).filter((row) =>
    (category === 'all' || row.category === category) &&
  ...
  filtered.sort((a, b) => sort === 'name'
    ...
    : sort === 'region'
      ? `${a.province} ${a.district} ${a.locality}`.localeCompare(
        `${b.province} ${b.district} ${b.locality}`, 'ko'
  ...
  ok(res, {
    ...
    stats: {
      ...
      maximumValuation,
      reconciliationPass:
        totalValuation === categoryBreakdown.reduce((sum, item) => sum + item.totalValuation, 0),
    },
    categoryBreakdown,
    subcategoryBreakdown,
    regionBreakdown,
    topOfficials,
    alpha: realEstateAlpha(year),
    meta: {
...
app.get('/api/alpha-engine', (req, res) => {
  const year = parseYear(req.query.year);
  const people = statement(`
    ${snapshotCte(year)}
    SELECT p.id, p.canonical_name AS name,
           d.organization_at_disclosure AS agency,
           d.title_at_disclosure AS title,
  ...
  for (const row of securityRows) {
    ...
    for (const holding of holdings) {
      if (!holding.canonicalName || Number(holding.quantity || 0) <= 0) continue;
  ...
  const realEstateRows = statement(`
    ${snapshotCte(year)}
  ...
  const usStockBulls = people
    ...
    .slice(0, 3);
  const alphaStocks = buildConcentration(
    stockHolders, smartIds, smartPopulation, publicPopulation, 2
  );
  const alphaRegions = buildConcentration(
    regionHolders, smartIds, smartPopulation, publicPopulation, 2
  );
  ...
  ok(res, {
    ...
    alphaRegions,
    usStockBulls,
    fxWhales: latestAssetLeaders(
      year,
      `a.category IN ('예금', '증권') AND
       (UPPER(a.detail) LIKE '%USD%' OR a.detail LIKE '%달러%' OR a.detail LIKE '%외화%')`
    ),
    bondWhales: latestAssetLeaders(
      year,
      `(a.category = '채권' OR
        (a.category = '증권' AND a.subcategory IN
          ('기타채권', '금융채', '회사채', '국채', '채권', '공채', '지방채')))`
    ),
    angelInvestors: latestAssetLeaders(
      year,
      `a.category = '증권' AND (a.subcategory LIKE '%비상장%' OR a.detail LIKE '%비상장%')`
    ),
    supercars: latestAssetLeaders(
      year,
      `a.category = '동산' AND
    ...
    vipMembers: latestAssetLeaders(
      year,
      `(a.category LIKE '%회원권%' OR a.subcategory LIKE '%회원권%')`
    ),
    preciousCollectors: latestAssetLeaders(
      year,
      `a.category IN ('금 및 백금', '보석류', '골동품 및 예술품')`
    ),
    listedStockWhales: latestAssetLeaders(
      year,
      `a.category = '증권' AND a.subcategory = '상장주식'`
    ),
    totalSecuritiesWhales: latestAssetLeaders(
      year,
      `a.category = '증권'`
    ),
    realEstateWhales: latestAssetLeaders(
      year,
      `a.category IN ('토지', '건물')`
    ),
    landWhales: latestAssetLeaders(year, `a.category = '토지'`),
    cryptoDisclosureWhales: latestAssetLeaders(year, `a.category = '가상자산'`),
    cashWhales: latestAssetLeaders(year, `a.category = '현금'`),
    equityStakeWhales: latestAssetLeaders(
      year,
      `a.category = '합명·합자·유한회사 출자지분'`
    ),
    quality: {
      cardCount: 16,
      deterministic: true,
...
app.listen(PORT, '127.0.0.1', () => {
  console.log(`K-Whale private API: http://127.0.0.1:${PORT}`);
